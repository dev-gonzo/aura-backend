...
import (
	...
	pedidoservice "sistema-editorial/editora/backend/src/pedidos/service"
)
...
func Register(app *fiber.App, svc service, requireAuth fiber.Handler, adminOnly fiber.Handler) {
	group := app.Group("/api/pedidos", requireAuth, adminOnly)
	...
	group.Get("/:id", func(c *fiber.Ctx) error {
		...
		if err != nil {
			...
			if pedidoservice.IsValidationError(err) {
	...
	group.Post("/", func(c *fiber.Ctx) error {
		...
		if err := c.BodyParser(&request); err != nil {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"message": "payload invalido para pedido"})
		}
	...
	group.Put("/:id", func(c *fiber.Ctx) error {
		...
		if err := svc.Update(ctx, c.Params("id"), request); err != nil {
			status := fiber.StatusInternalServerError
			if pedidoservice.IsValidationError(err) {
				status = fiber.StatusBadRequest
			}
